...
2. **Identify** the four scales of measurement (also known as data types) and select appropriate statistics for each
3. **Recognize** how data distribution shapes affect the choice of central tendency measures
4. **Understand** the basic logic of hypothesis testing and how to account for uncertainty in the data
...
![Central Tendency Measures. Source: [Learning Statistics with R](https://learningstatisticswithr.com/lsr-0.6.pdf)](images/image1.png)
...
### Example: Length of Stay for Patients
...
# Get the single mode
mode_table <- sort(table(los_example), decreasing = TRUE)
mode_los <- as.numeric(names(mode_table)[1])
```
...
ggplot(los_df, aes(x = LOS)) +
  ...
  geom_vline(xintercept = mode_los, color = "#4DAF4A", linewidth = 1.2, linetype = "dotted") +
  annotate("text", x = mean_los, y = 0.95, label = paste("Mean =", round(mean_los, 2)),
           color = "#E41A1C", hjust = 0, size = 4) +
  annotate("text", x = median_los, y = 0.85, label = paste("Median =", median_los),
           color = "#377EB8", hjust = 0, size = 4) +
  annotate("text", x = mode_los, y = 0.75, label = paste("Mode =", mode_los),
           color = "#4DAF4A", hjust = 0, size = 4) +
  scale_x_continuous(breaks = seq(0, 16, 2)) +
  labs(title = "Length of Stay: Mean vs. Median vs. Mode",
...
- **Mean**: (2 + 2 + 2 + 3 + 3 + 4 + 5 + 5 + 6 + 8 + 15) / 11 = 55 / 11 = **`r round(mean_los, 2)` days**
- **Median**: Arrange in order and find middle value (6th value) = **`r median_los` days**
...
Now let's take a look at how to calculate these measures in R.
  ...
  - `Mode()`: Returns the mode of a data vector (To use the `Mode()` function, we first need to load `DescTools` package)
...
# 4. Calculate Mode (we first need to load `DescTools` package))
library(DescTools)
...
calculation_table <- tibble(
  `Patient` = as.character(1:length(los_example)),
  `LOS (days)` = los_example,
  `Deviation from Mean (5 days)` = round(deviations, 2),
  `Squared Deviation` = round(squared_dev, 2)
)

# Add Sum and Average rows
calculation_table <- calculation_table %>%
  add_row(
    `Patient` = "Sum",
    `LOS (days)` = sum(los_example),
    `Deviation from Mean (5 days)` = round(sum(deviations), 2),
    `Squared Deviation` = sum(round(squared_dev, 2))
  ) %>%
  add_row(
    `Patient` = "Average (or Variance)",
    `LOS (days)` = mean(los_example),
    `Deviation from Mean (5 days)` = 0,
    `Squared Deviation` = round(variance_los, 2)
  )

kable(calculation_table, align = "c", escape = FALSE,
      col.names = c("Patient", "LOS (days)", "Deviation from Mean (5 days)", "Squared Deviation")) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), position = "center") %>%
  row_spec(12:13, bold = TRUE, background = "#e6f3ff")
```

**Calculation Steps**:

1. **Calculate mean**: `r sum(los_example)` / 11 = **`r round(mean_los_disp, 2)` days**
2. **Calculate deviations**: Each value minus the mean (see table above; the third column)
3. **Square the deviations**: Eliminates negative values, emphasizes larger deviations (the fourth column)
4. **Sum and average (Variance)**: Sum of squared deviations / (n-1) = **`r round(variance_los, 2)` days²**
5. **Take square root (SD)**: $\sqrt{\text{Variance}}$ = **`r round(sd_los, 2)` days**
...
- `table()`: Calculates frequencies (counts) for categorical data
- `prop.table()`: Calculates proportions (shares) from a table (i.e., combining `prop.table()` and `table()`)
- `range()`: Returns the minimum and maximum
- `var()`: Calculates the Variance (automatically uses n-1)
- `sd()`: Calculates the Standard Deviation (automatically uses n-1)
...
# Proportions (Shares)
prop.table(table(los_example))

# 2. Calculate Range
los_range <- range(los_example)
los_range 
# To get the single number (max - min):
diff(los_range)
...
#| tbl-cap: "Distribution of Insurance Types"
#| echo: false

insurance_df <- tibble(insurance_type = patient_satisfaction$insurance_type)
...
ggplot(patient_satisfaction, aes(x = factor(satisfaction_score))) +
  ...
  annotate("text", x = 6.5, y =  max(table(patient_satisfaction$satisfaction_score)) * 0.9,
...
::: {.callout-note}
## Special Note: Numeric Satisfaction and Pain Scales
...
ggplot(patient_satisfaction, aes(x = length_of_stay)) +
  ...
  annotate("text", x = mean_los + 7, y = max(table(patient_satisfaction$length_of_stay)) * 3,
...
ggplot(patient_satisfaction, aes(x = length_of_stay)) +
  ...
  annotate("text", x = mean_los_dist + 1, y = 160, label = paste("Mean =", round(mean_los_dist, 1)),
           color = "blue", hjust = 0, fontface = "bold") +
  annotate("text", x = median_los_dist - 1, y = 160, label = paste("Median =", median_los_dist),
...
For example, you may state: "The median length of stay is `r median_los_dist` days, meaning half of patients stay `r median_los_dist` days or less. However, the mean is `r round(mean_los_dist, 1)` days due to a small number of patients with extended stays."
...
ggplot(high_satisfaction, aes(x = satisfaction_rating)) +
  ...
  annotate("text", x = median_sat_ls + 2, y = 30, label = paste("Median =", round(median_sat_ls, 1)),
...
The result shows that most patients are highly satisfied, as shown by a median score of `r round(median_sat_ls, 1)`. However, the mean is lower (`r round(mean_sat_ls, 1)`) because a small group of very dissatisfied patients pulled the average down.
...
```{r}
#| label: tbl-shape-stats
#| tbl-cap: "Best Measures by Distribution Shape (Interval/Ratio Data)"
#| echo: false

shape_stats <- tibble(
  `Distribution Shape` = c("Normal (Symmetric)", "Right-Skewed (Positive)", "Left-Skewed (Negative)", "Bimodal"),
  Characteristics = c("Mounds in center, tails equal", "Long tail to right (high outliers)", "Long tail to left (low outliers)", "Two distinct peaks"),
  `Best Central Tendency` = c("Mean", "Median", "Median", "None (Analyze subgroups)"),
  Reasoning = c("Uses all data; efficient", "Not distorted by high outliers", "Not distorted by low outliers", "Overall average is misleading")
)
...
2. Could it just be due to random sampling?
3. If we surveyed 200 different patients, would we see the same pattern?
...
2. **Calculate** how likely our observed data would be if this assumption were true (this probability is called the **p-value**)
...
1. A p-value is the probability that H₀ is true (X)
...
The [0.05 threshold](https://en.wikipedia.org/wiki/P-value) was popularized by statistician **Ronald Fisher** in the 1920s. It's important to understand that this is **not based on mathematical proof**. It's simply a convention that the scientific community adopted as a reasonable balance between being too strict and too lenient.
...
Hospital C mean satisfaction = 5.2 [CI: 4.8, 5.6]
Hospital D mean satisfaction = 4.9 [CI: 4.5, 5.3]
...
- Lakens, D. (2014). [Improving Your Statistical Inferences; Chapter 1. Using p-values to test a hypothesis](https://lakens.github.io/statistical_inferences/01-pvalue.html) 

- Lakens, D. (2017). [Improving Your Statistical Inferences; Chapter 7. Confidence Intervals](https://lakens.github.io/statistical_inferences/07-CI.html) 
...
- Brown University. [Seeing Theory: Frequentist Inference](https://seeing-theory.brown.edu/frequentist-inference/index.html) - Beautiful interactive visualizations of confidence intervals, hypothesis testing, and the central limit theorem
